...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900">New Cash Advance</h1>
        </div>

        <form action="{{ route('cash-advances.store') }}" method="POST" class="space-y-5">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" min="{{ $settings['min_amount'] }}" max="{{ $settings['max_amount'] }}" required
                ...
                <div>
                    ...
                    <input type="number" name="term_months" min="1" max="{{ $settings['max_term'] }}" value="3" required
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm">
                </div>
                <div>
            ...
            <div class="flex gap-3">
                <a href="{{ route('cash-advances.index') }}" class="px-6 py-2.5 border border-gray-200 rounded-xl text-sm">Cancel</a>
