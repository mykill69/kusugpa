            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <h1 class="text-2xl font-bold text-gray-900">Loan #{{ $loan->loan_number }}</h1>
                        <span
                            class="px-3 py-1 text-xs font-medium rounded-full
                        @if ($loan->status == 'active') bg-green-100 text-green-700
                        @elseif($loan->status == 'pending') bg-yellow-100 text-yellow-700
                        @elseif($loan->status == 'approved') bg-blue-100 text-blue-700
                        @elseif($loan->status == 'completed') bg-gray-100 text-gray-700
                        @else bg-red-100 text-red-700 @endif">
                            {{ ucfirst($loan->status) }}
                        </span>
                    </div>
                    <p class="text-sm text-gray-500">{{ $loan->planter_name }} ({{ $loan->planter_code }})</p>
                </div>
                @php
                ...
                <div class="flex gap-2">
                    ...
                    @if ($canApprove && $loan->status === 'pending')
                        <button onclick="approveLoan()"
                            class="bg-green-600 text-white rounded-xl px-4 py-2 text-sm font-semibold hover:bg-green-700">
                            <i class="fas fa-check mr-1"></i> Approve
                        </button>
                        <button onclick="rejectLoan()"
                            class="bg-red-600 text-white rounded-xl px-4 py-2 text-sm font-semibold hover:bg-red-700">
                            <i class="fas fa-times mr-1"></i> Reject
                        </button>
                    @endif
                    @if ($canApprove && $loan->status === 'approved')
                        <form action="{{ route('loans.activate', $loan) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="bg-blue-600 text-white rounded-xl px-4 py-2 text-sm font-semibold hover:bg-blue-700">
                                <i class="fas fa-play mr-1"></i> Activate
                            </button>
                        </form>
                    @endif
                    @if ($canProcess && in_array($loan->status, ['pending', 'rejected']))
                        <form action="{{ route('loans.destroy', $loan) }}" method="POST"
                            onsubmit="return confirm('Delete this loan?')">
                            @csrf @method('DELETE')
                            <button type="submit"
                                class="bg-gray-600 text-white rounded-xl px-4 py-2 text-sm font-semibold hover:bg-gray-700">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-semibold">{{ $loan->loanType->name ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Principal</p>
                            <p class="text-sm font-semibold">₱{{ number_format($loan->principal_amount, 2) }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Interest Rate</p>
                            <p class="text-sm font-semibold">{{ $loan->interest_rate }}%</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Term</p>
                            <p class="text-sm font-semibold">{{ $loan->term_months }} months</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Monthly</p>
                            <p class="text-sm font-semibold">₱{{ number_format($loan->monthly_amortization, 2) }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Total Amount</p>
                            <p class="text-sm font-semibold">₱{{ number_format($loan->total_amount, 2) }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            <p class="text-xs text-gray-500">Crop Year</p>
                            <p class="text-sm font-semibold">{{ $loan->crop_year }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Start Date</p>
                            <p class="text-sm">{{ $loan->start_date ? $loan->start_date->format('M d, Y') : 'N/A' }}</p>
                        </div>
                    </div>
                    @if ($loan->purpose)
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <p class="text-xs text-gray-500">Purpose</p>
                            <p class="text-sm">{{ $loan->purpose }}</p>
                        </div>
                    @endif
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    @if (in_array(auth()->user()->role, ['Administrator', 'super_admin', 'supervisor', 'loan_officer']))
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($loan->amortizations as $amort)
                                    <tr class="hover:bg-gray-50/50">
                                        ...
                                        <td class="px-3 py-2 text-sm text-right">
                                            ₱{{ number_format($amort->amount_due, 2) }}</td>
                                        <td class="px-3 py-2 text-sm text-right text-green-600">
                                            ₱{{ number_format($amort->amount_paid, 2) }}</td>
                                        <td class="px-3 py-2 text-sm text-right">
                                            ₱{{ number_format($amort->balance_after, 2) }}</td>
                                        ...
                                        @if (in_array(auth()->user()->role, ['Administrator', 'super_admin', 'supervisor', 'loan_officer']) &&
                                            ...
                                            <td class="px-3 py-2 text-center">
                                                <button
                                                    onclick="recordPayment({{ $amort->id }}, {{ $amort->amount_due }})"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-4 py-8 text-center text-gray-500">No amortization
                                            schedule yet</td>
                                    </tr>
                                @endforelse
                            </tbody>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-2">
                        <a href="{{ route('loans.amortization-pdf', $loan) }}" target="_blank"
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-semibold text-gray-900">Amortization Schedule</p>
                                <p class="text-xs text-gray-500">Payment schedule breakdown</p>
                            </div>
                            <i class="fas fa-download text-gray-400 group-hover:text-blue-600 transition-colors"></i>
                        </a>

                        <a href="{{ route('loans.details-pdf', $loan) }}" target="_blank"
                            class="flex items-center gap-3 p-3 rounded-xl bg-green-50 border border-green-100 hover:bg-green-100 transition-colors group">
                            <div
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-500">Complete loan information</p>
                            </div>
                            <i class="fas fa-download text-gray-400 group-hover:text-green-600 transition-colors"></i>
                        </a>

                        <a href="{{ route('loans.soa-pdf', $loan) }}" target="_blank"
                            class="flex items-center gap-3 p-3 rounded-xl bg-purple-50 border border-purple-100 hover:bg-purple-100 transition-colors group">
                            <div
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-semibold text-gray-900">Statement of Account</p>
                                <p class="text-xs text-gray-500">Payment history & balance</p>
                            </div>
                            <i class="fas fa-download text-gray-400 group-hover:text-purple-600 transition-colors"></i>
                        </a>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between py-2 border-b border-gray-50">
                            ...
                            <span
                                class="text-sm font-semibold text-gray-900">{{ $loan->application_date->format('M d, Y') }}</span>
                        </div>
                        @if ($loan->approved_date)
                            <div class="flex items-center justify-between py-2 border-b border-gray-50">
                                <span class="text-sm text-gray-500">Approved</span>
                                <span
                                    class="text-sm font-semibold text-gray-900">{{ $loan->approved_date->format('M d, Y') }}</span>
                            </div>
                        @endif
                        @if ($loan->approvedByUser)
                            <div class="flex items-center justify-between py-2 border-b border-gray-50">
                                <span class="text-sm text-gray-500">Approved By</span>
                                <span class="text-sm font-semibold text-gray-900">{{ $loan->approvedByUser->fname }}
                                    {{ $loan->approvedByUser->lname }}</span>
                            </div>
                        @endif
                        <div class="flex items-center justify-between py-2">
                            <span class="text-sm text-gray-500">Created By</span>
                            <span class="text-sm font-semibold text-gray-900">{{ $loan->createdByUser->fname }}
                                {{ $loan->createdByUser->lname }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form id="paymentForm" method="POST">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="number" name="amount_paid" id="paymentAmount" step="0.01" required
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Week Number</label>
                        <input type="text" name="week_no" placeholder="e.g., 22"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm">
                    </div>
                </div>
                <div class="flex gap-3 mt-6">
                    ...
                    <button type="submit"
                        class="flex-1 px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-semibold">Save</button>
                </div>
            </form>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            function recordPayment(amortizationId, amountDue) {
                ...
                document.getElementById('paymentForm').action = '{{ route('loans.payment', $loan) }}';
